    ...
    <section className="bg-transparent px-4 sm:px-8 pb-0">
      <ContainerScroll className="w-full h-[140vh]" totalCards={myAccomplishments.length}>
        <div className="sticky left-0 top-0 h-svh w-full flex items-center justify-center overflow-hidden">
          {/* HARD SHRINKING ON MOBILE (w-[75vw] max-w-[280px]) TO PREVENT CORNER CLIPPING ON ROTATION */}
          <CardsContainer className="mx-auto w-[75vw] max-w-[280px] sm:max-w-[320px] md:max-w-2xl lg:max-w-3xl h-[380px] md:h-[500px]">
            {myAccomplishments.map((item, index) => (
              <CardTransformed
                key={item.tempId}
                className="items-start justify-evenly border border-stone-200/50 dark:border-[#d5c0aa]/15 bg-white dark:bg-[#0a0a0a] shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.8)] px-5 py-8 md:px-12 md:py-10"
                arrayLength={myAccomplishments.length}
                index={index + 1}
              >
                <div className="flex flex-col items-center justify-center space-y-4 md:space-y-6 w-full h-full text-center">
                  <div className="relative mb-2 w-[120px] h-[120px] md:w-[180px] md:h-[180px]">
                      <Image
                          src={item.image}
                          alt={item.title}
                          fill
                  ...
                  <div>
                    <h3 className="text-xl md:text-4xl font-serif font-bold text-stone-800 dark:text-[#d5c0aa] drop-shadow-sm tracking-wide leading-tight">
                      {item.title}
                    </h3>
                  </div>
                  <p className="text-stone-500 dark:text-stone-300 font-light leading-relaxed max-w-[220px] md:max-w-[420px] text-sm md:text-xl">
                    {item.desc}
                  </p>
                </div>
              </CardTransformed>
