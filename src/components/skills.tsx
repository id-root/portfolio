...
const SkillItem = ({ item, index }: { item: string, index: number }) => {
    ...
    return (
        <motion.li
            ref={itemRef}
            style={{ y }}
            className="group/skill relative py-4 border-b border-beige-200/40 dark:border-stone-700/40 last:border-0 text-text-secondary hover:text-accent-sienna transition-colors duration-300 font-outfit cursor-default"
        >
            {/* Left accent bar — appears on hover */}
            <div className="absolute left-0 inset-y-0 w-0.5 rounded-full bg-transparent group-hover/skill:bg-accent-caramel transition-all duration-300 opacity-0 group-hover/skill:opacity-100" />

            <span className="inline-block pl-0 group-hover/skill:pl-4 transition-all duration-300">
                {item}
            </span>

            {/* Subtle glow on hover */}
            <div className="absolute inset-0 bg-gradient-to-r from-accent-caramel/5 to-transparent opacity-0 group-hover/skill:opacity-100 transition-opacity duration-300 pointer-events-none rounded" />
        </motion.li>
    );
};
...
export const Skills = () => {
    ...
    return (
        <section id="capabilities" className="py-32 px-4 md:px-8 relative bg-beige-50/50 dark:bg-transparent">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-accent-caramel/[0.02] to-transparent pointer-events-none" />
