        ...
        if (trimmed === "help") {
            ...
            output = (
                <div className="text-stone-300 space-y-1">
                    ...
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> Anti-Debug-Framework/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> Aegis/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> MultiLang-Evasion/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> sqli-hunter/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> Synapse/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> spectre/</p>
                    <p><span className="text-[var(--brand-accent)]">drwxr-xr-x</span> Isotope/</p>
                </div>
            );
        ...
        <div className="min-h-screen relative transition-colors duration-300 flex flex-col">
            ...
            <main className="flex-1 flex items-center justify-center pt-24 pb-12 px-4">
                <motion.div
                    ...
                    className="w-full max-w-4xl bg-[var(--brand-primary)] border border-border/30 rounded-xl overflow-hidden shadow-[0_20px_60px_rgba(49,39,38,0.3)] flex flex-col h-[600px] relative"
                    onClick={() => inputRef.current?.focus()}
                >
                    {/* Doodle: Sparkle accents near terminal corners */}
                    <div className="absolute -top-6 -right-6 z-30">
                        <StarSparkle size={45} color="var(--brand-accent)" delay={1} />
                    </div>
                    <div className="absolute -bottom-4 -left-4 z-30">
                        <StarSparkle size={30} color="var(--brand-accent)" delay={1.5} />
                    </div>
                    {/* CRT Scanline overlay */}
                    <div
                        className="absolute inset-0 pointer-events-none z-20 opacity-[0.05]"
                        style={{
                            backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 1px, rgba(196,168,130,0.3) 1px, rgba(196,168,130,0.3) 2px)',
                            backgroundSize: '100% 2px',
                        }}
                    />

                    {/* Header */}
                    <div className="bg-[var(--brand-primary)] px-4 py-3 flex items-center justify-between border-b border-border/30 relative z-10 shadow-sm">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="relative flex-1 h-6">
                                    ...
                                    <span className="absolute top-0 left-0 text-stone-300 pointer-events-none whitespace-pre z-0">
                                        {input}
                                        <span className="animate-pulse bg-[var(--brand-accent)] text-[var(--brand-dark)] inline-block w-2.5 h-4 align-middle ml-1"> </span>
                                    </span>
                                </div>
                            </div>
                        )}
